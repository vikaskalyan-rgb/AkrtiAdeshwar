...
export default function FlatManagement() {
  ...
  const handleSave = async () => {
    if (!form.ownerName && form.ownerType !== 'VACANT') return
    setSaving(true)
    try {
      ...
      <div className="flex-1 overflow-y-auto p-3 md:p-5 space-y-3">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-3">
          {[
            ...
            <div key={s.label} onClick={s.onClick}
              ...
              <div className="text-[28px] font-bold mt-0.5" style={{ color:s.color, letterSpacing:'-0.03em' }}>{s.value}</div>
            </div>
          ))}
        </div>
        ...
        <div className="card p-4">
          <div className="flex justify-between items-center mb-2">
            <span className="text-[12px] font-semibold" style={{ color:'var(--ink-2)' }}>Data Completion</span>
            <span className="text-[12px] font-bold" style={{ color:'var(--emerald)' }}>
              {flats.length > 0 ? Math.round(((flats.length - stats.unknown) / flats.length) * 100) : 0}%
            </span>
          </div>
          <div className="h-2 rounded-full overflow-hidden" style={{ background:'var(--surface-3)' }}>
            <div className="h-full rounded-full transition-all"
              style={{
                width: `${flats.length > 0 ? ((flats.length - stats.unknown) / flats.length) * 100 : 0}%`,
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <div className="flex gap-1 rounded-xl p-1 flex-shrink-0" style={{ background:'white', border:'1px solid var(--border)' }}>
            {[
              ['all','All'],
              ['unknown','Pending'],
              ['occupied','Occupied'],
          ...
          <div className="card overflow-hidden">
            ...
            <div className="hidden md:grid text-[10px] font-bold uppercase tracking-wide px-5 py-3"
              style={{ gridTemplateColumns:'80px 100px 1fr 1fr 140px 44px',
                background:'var(--surface-3)', color:'var(--ink-3)', borderBottom:'1px solid var(--border)' }}>
              <span>Flat</span>
              <span>Type</span>
              <span>Owner</span>
            ...
            <div className="overflow-y-auto" style={{ maxHeight:'calc(100dvh - 380px)' }}>
              {filtered.map(f => {
                ...
                return (
                  <div key={f.flatNo}
                    ...
                    <div className="flex items-center gap-3 px-4 py-3 md:hidden">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          {unknown && (
                            <span className="badge text-[9px]" style={{ background:'#ffe4e6', color:'#9f1239' }}>
                              Info needed
                            </span>
                          )}
                        </div>
                      ...
                      <button onClick={() => openEdit(f)}
                        ...
                        {justSaved ? <CheckCircle2 size={16} style={{ color:'var(--emerald)' }} /> : <Edit2 size={15} />}
                      </button>
                    </div>
                    ...
                    <div className="hidden md:grid items-center px-5 py-3"
                      style={{ gridTemplateColumns:'80px 100px 1fr 1fr 140px 44px' }}>
                      <span className="text-[13px] font-bold font-mono" style={{ color: ts.color }}>{f.flatNo}</span>
                      <span>
                        <span className="badge text-[9px]" style={{ background:ts.bg, color:ts.color }}>{ts.label}</span>
                      </span>
                      <div className="min-w-0 pr-3">
                        {unknown
                          ? <span className="badge text-[9px]" style={{ background:'#ffe4e6', color:'#9f1239' }}>Info needed</span>
                          : <>
                            <div className="text-[12px] font-medium truncate" style={{ color:'var(--ink)' }}>{f.ownerName}</div>
                            <div className="text-[10px]" style={{ color:'var(--ink-3)' }}>{f.ownerPhone || '—'}</div>
                          </>
                        }
                      </div>
                      <div className="min-w-0 pr-3">
                        {f.ownerType === 'RENTED'
                          ? <>
                            <div className="text-[12px] font-medium truncate" style={{ color:'var(--ink)' }}>{f.residentName || '—'}</div>
                            <div className="text-[10px]" style={{ color:'var(--ink-3)' }}>{f.residentPhone || '—'}</div>
                          </>
                      ...
                      <span className="text-[11px]" style={{ color:'var(--ink-3)' }}>{f.parkingSlot || '—'}</span>
                      <button onClick={() => openEdit(f)}
                        className="w-8 h-8 flex items-center justify-center rounded-lg transition-all"
                        style={{ background: justSaved ? '#ecfdf5' : 'var(--indigo-lt)', color: justSaved ? 'var(--emerald)' : 'var(--indigo)' }}>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {OWNER_TYPES.map(t => (
                  <button key={t.value} onClick={() => setForm(f => ({ ...f, ownerType: t.value }))}
            ...
            <div className="rounded-xl p-4 space-y-3"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[11px] font-semibold mb-1.5 block" style={{ color:'var(--ink-2)' }}>Full Name</label>
                  <input className="input" placeholder="Owner name"
                    value={form.ownerName} onChange={e => setForm(f=>({...f, ownerName:e.target.value}))} />
                </div>
                <div>
                  <label className="text-[11px] font-semibold mb-1.5 block" style={{ color:'var(--ink-2)' }}>Phone</label>
                  <input className="input" placeholder="10-digit mobile" maxLength={10}
                    value={form.ownerPhone} onChange={e => setForm(f=>({...f, ownerPhone:e.target.value.replace(/\D/,'')}))} />
                </div>
                <div className="col-span-2">
                  <label className="text-[11px] font-semibold mb-1.5 block" style={{ color:'var(--ink-2)' }}>Email (optional)</label>
                  <input className="input" placeholder="owner@email.com"
                    value={form.ownerEmail} onChange={e => setForm(f=>({...f, ownerEmail:e.target.value}))} />
                </div>
              </div>
            </div>

            {/* Tenant info — only for rented */}
            {form.ownerType === 'RENTED' && (
              <div className="rounded-xl p-4 space-y-3"
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-[11px] font-semibold mb-1.5 block" style={{ color:'var(--ink-2)' }}>Full Name</label>
                    <input className="input" placeholder="Tenant name"
                      value={form.residentName} onChange={e => setForm(f=>({...f, residentName:e.target.value}))} />
                  </div>
                  <div>
                    <label className="text-[11px] font-semibold mb-1.5 block" style={{ color:'var(--ink-2)' }}>Phone</label>
                    <input className="input" placeholder="10-digit mobile" maxLength={10}
                      value={form.residentPhone} onChange={e => setForm(f=>({...f, residentPhone:e.target.value.replace(/\D/,'')}))} />
                  </div>
                  <div className="col-span-2">
                    <label className="text-[11px] font-semibold mb-1.5 block" style={{ color:'var(--ink-2)' }}>Email (optional)</label>
                    <input className="input" placeholder="tenant@email.com"
                      value={form.residentEmail} onChange={e => setForm(f=>({...f, residentEmail:e.target.value}))} />
                  </div>
                </div>
              </div>
            )}
            ...
            <div>
              <label className="text-[11px] font-bold uppercase tracking-wide block mb-1.5"
                style={{ color:'var(--ink-2)' }}>Parking Slot (optional)</label>
              <input className="input" placeholder="e.g. P12"
                value={form.parkingSlot} onChange={e => setForm(f=>({...f, parkingSlot:e.target.value}))} />
            </div>
